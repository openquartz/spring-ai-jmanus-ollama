...
import com.openquartz.cloud.ai.example.manus.dynamic.model.service.ModelService;
...
public class InitController {
	...
	public ResponseEntity<Map<String, Object>> getInitStatus() {
		...
		try {
			...
			// Support OllamaApi compatible API configuration
			String OllamaApiKeyFromEnv = environment.getProperty("OllamaApi_API_KEY");
			boolean hasOllamaApiEnvConfig = OllamaApiKeyFromEnv != null && !OllamaApiKeyFromEnv.trim().isEmpty();

			// If any environment variable is configured, consider it as configured
			boolean hasEnvConfig = hasDashscopeEnvConfig || hasOllamaApiEnvConfig;
			...
			response.put("hasOllamaApiEnvConfig", hasOllamaApiEnvConfig);
	...
	private ModelConfig createOrUpdateDefaultModel(InitConfigRequest request) {
		try {
			...
			if ("dashscope".equals(configMode)) {
				...
				String description = "Custom OllamaApi compatible model - " + displayName
